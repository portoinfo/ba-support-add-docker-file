    ...
    <b-container fluid>
        ...
        <b-row class="mb-3 px-2" cols='1' cols-lg='3' align-h="between" align-v="center" no-gutters>
            ...
            <b-col class="py-2 px-lg-2 text-right">
                <b-button @click="listDepartment" variant="btn bs-btn-back btn-block-md">
                    <i class="fa fa-bars" aria-hidden="true"></i>&nbsp;{{btnDepartmentList}}
                </b-button>
            </b-col>
            <b-col class="py-2 px-lg-2">
                <b-button @click="createDepartment" variant="btn bs-btn-add btn-block">
                    <i class="fa fa-plus secondary" aria-hidden="true"></i> {{btnNewDepartment}}
                </b-button>
            </b-col>
        </b-row>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                subtitle: this.$t('bs-department-dashboard'),
                btnNewDepartment: this.$t('bs-register-new-department'),
                btnDepartmentList: this.$t('bs-list-of-departments'),
                ...
                placeholderSelectAttendant: this.$t('bs-select-attendant'),
                selectedAttendant: [],
        ...
        methods: {
            createDepartment: function() {
                //redirect
                window.open(`${this.base_url}/department/list-departments?new=department`, '_self')
            },
            listDepartment: function() {
                //redirect
                window.open(`${this.base_url}/department/list-departments`, '_self')
            },

            getDepartments: async function(){
                let url = `${this.base_url}/department/get-departments-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
            ...
            getAttendants: async function(department){
                let url = `${this.base_url}/department/get-attendants-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
                    ...
                    if(response.data.success){
                        ...
                        this.$set(this.$data, 'selectAttendantOptions', response.data)
                        this.$set(this.$data, 'selectedAttendant', this.selectAttendantOptions.slice(0, 5))
            ...
            getSummaryCards: async function(department){
                let url = `${this.base_url}/department/get-totals-and-percentages-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
            ...
            getAvgSummaryCards: async function(department){
                let url = `${this.base_url}/department/get-general-avg-for-attendances-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
            ...
            refreshPageData: async function(department) {
                if(!this.isFirstLoad) {
                    this.isFirstLoad = true
                    this.$spinner.show()
                }
                await this.getSummaryCards(department)
                await this.getAvgSummaryCards(department)
                await this.loadBarChart(this.selectedPeriod, department)
                await this.getAttendants(department) // e preciso carregar os atendentes antes do grafico de linha
                await this.loadLineChart(this.selectedPeriod2, this.selectedAttendant, department)
                await this.getTicketTimeCharts(this.ticketPeriodSelected, department)
                await this.getChatTimeCharts(this.chatPeriodSelected, department)
                if(this.isFirstLoad){
                    this.isFirstLoad = false
                    this.$spinner.hide()
                }
            },
            loadWeekBarChart: async function(department) {
                let url = `${this.base_url}/department/get-bar-chart-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
            ...
            loadMonthBarChart: async function(department) {
                let url = `${this.base_url}/department/get-bar-chart-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
            ...
            loadYearBarChart: async function(department) {
                let url = `${this.base_url}/department/get-bar-chart-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
            ...
            loadBarChart: async function (type, department) {
                this.chart1.loadedData = false
                switch(type.toLowerCase()) {
                    case 'week':
                    default:
                        await this.loadWeekBarChart(department)
                        break;
                    case 'month':
                        await this.loadMonthBarChart(department)
                        break;
                    case 'year':
                        await this.loadYearBarChart(department)
                        break;
                }
            },
            getBgColor(index) {
            ...
            loadWeekLineChart: async function(attendants, department) {
                if(
                    (attendants === undefined || attendants === null || attendants.length == 0)
                    || ( department === undefined || department === null )
                ){
                    this.chart2.loadedData = true
                    return
                }
                let url = `${this.base_url}/department/get-line-chart-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
                    let response = await axios.post(url, {
                        company_id: this.csid,
                        department_id: department,
                        attendant_ids: attendants.map(el => el.value),
                    ...
                    if(response.data.success){
                        ...
                        for(let key in response.data.attendants) {
                            datasets.push({
                                label: this.getLabel(key, attendants),
            ...
            loadMonthLineChart: async function(attendants, department) {
                if(
                    (attendants === undefined || attendants === null || attendants.length == 0)
                    || ( department === undefined || department === null )
                ){
            ...
            loadYearLineChart: async function (attendants, department) {
                if(
                    (attendants === undefined || attendants === null || attendants.length == 0)
                    || ( department === undefined || department === null )
                ){
            ...
            loadLineChart: async function (type, attendants, department) {
                this.chart2.loadedData = false
                switch(type.toLowerCase()) {
                    case 'week':
                    default:
                        await this.loadWeekLineChart(attendants, department)
                        break;
                    case 'month':
                        await this.loadMonthLineChart(attendants, department)
                        break;
                    case 'year':
                        await this.loadYearLineChart(attendants, department)
                        break;
                }
            },
            getTicketTimeCharts: async function(period, department) {
                let url = `${this.base_url}/department/get-ticket-time-cards-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
            ...
            getChatTimeCharts: async function(period, department) {
                let url = `${this.base_url}/department/get-chat-time-cards-department-dashboard`
                if(!this.isFirstLoad) {
                    this.$spinner.show();
                }
                try{
        ...
        watch: {
            selectedDepartment: async function (newVal, oldVal) {
                await this.refreshPageData(newVal)
            },
            selectedPeriod: function (newVal, oldVal) {
                this.loadBarChart(newVal, this.selectedDepartment)
            },
            selectedPeriod2: function (newVal, oldVal) {
                this.loadLineChart(newVal, this.selectedAttendant, this.selectedDepartment)
            },
            selectedAttendant: function (newVal, oldVal) {
                if(oldVal.length == 0) { return }
                this.loadLineChart(this.selectedPeriod2, newVal, this.selectedDepartment)
            },
            ticketPeriodSelected: function (newVal, oldVal) {
                this.getTicketTimeCharts(newVal, this.selectedDepartment)
            },
            chatPeriodSelected:  function (newVal, oldVal) {
                this.getChatTimeCharts(newVal, this.selectedDepartment)
            }
        },
        created() {
            this.getDepartments()
