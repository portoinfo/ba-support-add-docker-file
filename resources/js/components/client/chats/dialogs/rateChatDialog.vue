	...
	<v-dialog v-model="rateDialogShown" scrollable persistent max-width="600" content-class="custom-dialog">
        <v-card class="card-dialog" color="whiteCard" elevation="15">
            ...
            >
                    ...
                    <v-container>
                        <v-row :no-gutters="$store.state.isPopup">
...
export default {
	...
	computed: {
		rateDialogShown: {
			get() {
				return this.$root.$refs.ClientChatOpened.rateDialogShown;
			},
			set(value) {
				this.$root.$refs.ClientChatOpened.rateDialogShown = value;
			},
		},
        evaluated: {
			get() {
			...
			set(value) {
				this.$root.$refs.ClientChatOpened.evaluated = value;
			},
		},
        typeEvaluation() {
            if ('evaluation' in this.$root.$refs.ClientChatOpened.departmentSettings) {
                var typeevaluation = this.$root.$refs.ClientChatOpened.departmentSettings.evaluation.typeevaluation;
        ...
        settingsAttenActive() {
            if ('evaluation' in this.$root.$refs.ClientChatOpened.departmentSettings) {
                return this.$root.$refs.ClientChatOpened.departmentSettings.evaluation.text_atten_active;
            }
        },
        settingsServActive() {
            if ('evaluation' in this.$root.$refs.ClientChatOpened.departmentSettings) {
                return this.$root.$refs.ClientChatOpened.departmentSettings.evaluation.text_serv_active;
            }
        },
        settingsCommentActive() {
            if ('evaluation' in this.$root.$refs.ClientChatOpened.departmentSettings) {
                return this.$root.$refs.ClientChatOpened.departmentSettings.evaluation.text_comment_active;
            }
        },
        rateFormAction() {
            return this.$root.$refs.ClientChatOpened.rateFormAction;
        }
	},
    methods: {
        submit() {
            if (this.$refs.formEvaluation.validate()) {
                switch (this.rateFormAction) {
                    case 'RATE':
                        this.$root.$refs.ClientChatOpened.rateChat(this.form).then(() => {
