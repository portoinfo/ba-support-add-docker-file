<template>
	<div class="modal-custom bordateste">
        <div class="modal-header border-0 p-0 mb-1">   
			<h5 @click="closeModal" class="modal-title ml-2 caret" id="exampleModalLabel">
				<i class="fa fa-angle-double-right fa-2x" aria-hidden="true"></i> Base de dados
			</h5>
            <!-- <i class="fa fa-times fa-2x caret" @click="closeModal" aria-hidden="true"></i> -->
        </div>
						...
						<table class="table">
							<tbody>
								<tr v-for="(item, index) in TicketorChat" :key="index" :value="item.value" >
									<th class="something" scope="row" @click="getChatHistory(item)">{{item.ticket_id}}</th>
...
<style scoped>
    ...
    .modal-custom {
        ...
        box-shadow: 2px 2px 2px 2px rgba(119, 119, 119, 0.829);
		position: fixed;
		width: 100%;
		overflow: hidden;
    }
