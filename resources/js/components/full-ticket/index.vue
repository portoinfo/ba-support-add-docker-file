<template>
  <div class="h-100 zoom-80" :class="{'with-tab': numberOfTabs >= 1 }">
    ...
    <b-col
      class="left-sidebar"
      v-show="!isMobile || (isMobile && showMenuMobile)"
      >
        ...
        <ul class="list-group myUL">
            <li class="list-group-item pr-0 pl-0 pb-0 br5 li-body">
              ...
              <ul class="list-group">
                  ...
                  >
                  <i
                  v-if="showdrop_ActiveTickets"
                  class="material-icons local_arrow"
                  >keyboard_arrow_down</i
                  >
                  <i v-else class="material-icons local_arrow"
          ...
          <ul v-show="showdrop_ActiveTickets" class="list-group">
              ...
              >
                <span class="level-3">
                  <i class="material-icons local_arrow">query_builder</i
                    >&nbsp;{{ $t("bs-opened")}}&nbsp;{{ countfixed.open > 0 ? `(${countfixed.open})` : '' }}
                </span>
                <span v-if="notify.onQueue" class="badge badge-primary badge-pill">on</span>
              </li>
            <li@click="markAsActive(1)" :class="[ 'list-group-item caret li-hover', activeMenuControls.active.inProgress ? 'selected' : '',]">
            <span class="level-3">
            <i class="material-icons local_arrow">question_answer</i
              >&nbsp;{{ $t("bs-in-progress")
            }} &nbsp;{{ countfixed.in_progress > 0 ? `(${countfixed.in_progress})` : '' }}
            </span>
            <span v-if="notify.inProgress" class="badge badge-primary badge-pill">on</span>
            </li>
            ...
            <li @click="markAsActive(2)" :class="[ 'list-group-item caret li-hover', activeMenuControls.active.overdue ? 'selected' : '', ]">
              <span class="level-3">
                <i class="material-icons local_arrow">report_problem</i>&nbsp;{{$t('bs-overdue')}} &nbsp;{{ countfixed.overdue > 0 ? `(${countfixed.overdue})` : '' }}
            </span>
            </li>
          </ul>
          </li>
          ...
          <span v-if="notify.closed" class="badge badge-primary badge-pill">on</span>
          </li>
          </span>
          <li
          ...
          <span v-if="notify.resolved" class="badge badge-primary badge-pill">on</span>
          </li>
          <li
          ...
          <span v-if="notify.canceled" class="badge badge-primary badge-pill">on</span>
          </li>
          <li
                  ...
                  <ul v-show="showdrop_MyTickets" class="list-group">
                    ...
                    <li @click="markAsActive(7);" :class="['list-group-item caret li-hover', activeMenuControls.myTickets.myActiveGroups ? 'selected' : '']">
                      <span class="level-3">
                        ...
                        v-if="!ddStatusMyTickets"
                        name="checkbox-1"
                        class="ml-5 mb-2 mt-2 fz-18"
                        >
                        {{ $t("bs-my-tickets") }}
                    </b-form-checkbox>
                    <li
    ...
    <b-col
    v-show="
    (isMobile && !showMenuMobile && showListTicket) ||
    (!isMobile && showListTicket)
    "
    class="ml-n3 h-90"
    >
    ...
    <tbody>
      ...
      <template v-for="(item, key) in tickets">
          ...
          >
          <td>
              <b-button size="sm" @click="openTicket(item)">{{ $t("bs-take").toUpperCase() }}</b-button
              >&nbsp;
              #{{ item.id }}
          </td>
      ...
      <!-- IN PROGRESS  -->
        <tr
          ...
          <td v-bind:class="{ answered: item.answered }">{{ $t(item.name_created) }}</td>
          <td v-bind:class="{ answered: item.answered }">{{ UTCtoClientTZ2_translated(item.created_at, tz) }}</td>
          <td v-bind:class="{ answered: item.answered }" :id="'time-elapsed-progress-' + item.id">
              {{
                calculateWaitingTime(
                UTCtoClientTZ(
                item.created_at,
        ...
        >
          ...
          <td v-bind:class="{ answered: item.answered }">{{ $t(item.name_created) }}</td>
          <td v-bind:class="{ answered: item.answered }">{{ UTCtoClientTZ2_translated(item.created_at, tz) }}</td>
          <td v-bind:class="{ answered: item.answered }" :id="'time-elapsed-overdue-' + item.id">
            {{
              calculateWaitingTime(
              UTCtoClientTZ(
              item.created_at,
      ...
      <div slot="no-results" class="mt-2 mb-2">
        ...
        <div class="row ml-2" style="text-aling:right">
          ...
          >
              ...
              <template #first-text><span class="text-dark">{{ $t('bs-first')}}</span></template>
              <template #prev-text><span class="text-dark">{{ $t('bs-prev')}}</span></template>
              <template #next-text><span class="text-dark">{{ $t('bs-next')}}</span></template>
              <template #last-text><span class="text-dark">{{ $t('bs-last')}}</span></template>
          ...
          <span style="margin-top:28px;">
            <span style="opacity: 0.6;width:50px;">
              Quantidade de tickets por pagina
              <input style="opacity: 0.8;width:50px;" @click="setQuantidadePorPagina" v-model="quantRows" type="number" value="8">
            </span>
          </span>
        </div>
    ...
    <b-col v-show="showFilters && !isMobile" cols="2" class="ml-n3 h-100">
      <b-card show class="h-100 ticket-information" header="Procurar" header-tag="header">
        <!-- FILTER CARD -->
        <filter-all
        :is_admin="is_admin"
        :session_user="user"
...
export default {
  data() {
    return {
      ...
      //badge notifications
      notify: {
        audio: new Audio("/media/notification.mp3"),
        onQueue: false,
        inProgress: false,
        transferred: false,
        closed: false,
      ...
      clientChatHistory: [],
      clientTicketHistory: [],
      statusMyTickets: true,
      ddStatusMyTickets: false,
      ticketoriginal: [],
      TicketRetorno: [],
      infiniteId: +new Date(),
      page: 1,
...
created() {
    ...
    .catch(err => {
      ...
      Echo.join(`tickets-list.${this.cs.id}`).listen(
        "TicketsListUpdate",
        (event) => {
          //console.log(event);
              ...
              case "OPENED":
                ...
                if (this.user_departments_id.includes(event.ticket.company_department_id)) {
                  ...
                  if (this.activeMenuControls.active.opened) {
                      ...
                      if(this.currentPage == this.totalPages){ // SÓ DA O PUSH SE ESTIVER NA ULTIMA PAGINA
              ...
              default:
                ...
                let index = this.tickets.findIndex(
                    ...
                    if(event.ticket.status == 'OPENED'){
                        ...
                        // if(event.ticket.status_final == 'IN_PROGRESS'){
    ...
    Echo.join(`tickets-agent-list.${this.cs.id}.${this.user.id}`).listen(
      ...
      (event) => {
        this.updateListTicket(event);
    });

    // canal para tickets em progresso, fechados, resolvidos, cancelados e transferidos para outro atendente.
    Echo.join(`tickets-list-full-ticket.${this.cs.id}`).listen(
      "FullTicket",
      (event) => {
    ...
    if(this.restriction[0].ticket_admin || this.restriction[0].ticket_alllist){
      this.statusMyTickets = false;
      this.ddStatusMyTickets = false;
    }else{
      this.statusMyTickets = true;
      this.ddStatusMyTickets = true; 
    }
  ...
  updateListTicket(event){
        ...
        case "push":
			...
			case "IN_PROGRESS":
				if (this.activeMenuControls.active.inProgress) {
					if(this.currentPage == this.totalPages){ // SÓ DA O PUSH SE ESTIVER NA ULTIMA PAGINA
                        if(this.statusMyTickets){ // VERIFICA SE MEUS TICKETS ESTÃO ATIVADOS, SE NÃO FAZ O PUSH
                            if(event.ticket.data.email == this.user.email){// VERIFICA SE EU SOU O ATENDENTE, SE SIM FAZ O PUSH
                                this.tickets.push(event.ticket.data);
                            }
                        }else{
                            this.tickets.push(event.ticket.data);
    ...
    loadTicket($state) {
        ...
        if (this.activeMenuControls.active.opened) {
            ...
            //this.myTicketsSearch($state, "tickets");
        } else if (this.activeMenuControls.myTickets.myActiveGroups) {
            //this.myTicketsSearch($state, "groups");
...
openTicket(value) {
    ...
    axios.post(`ticket-add-chat-admin`, {
        ...
        if(response.data.success){
          ...
          vm.itemselected.update_status_in_progress = response.data.update_status_in_progress;
          vm.itemselected.email = response.data.email;
          }
          vm.title = "";
        }else{
    ...
    searchTicket($state, type = "OPENED") { //gettickets
        var vm = this;

        if(vm.statusMyTickets){
            var vm = this;

            axios.get("tickets/get-my-tickets", {
                params: {
                    activeMenuControls: vm.activeMenuControls,
                    page: vm.page,
                    skip: vm.skip,
                ...
                if(vm.activeMenuControls.active.opened) {
                  ...
                  if (r_resposta.data.result.data.length > 0) {
                      //vm.tickets.push(...r_resposta.data.result.data);
                      vm.skip = r_resposta.data.skip;
                      vm.tickets = r_resposta.data.result.data;
                  }
                }
            ...
            .then(function (r_resposta) {
                // console.log(vm.restriction[0].ticket_admin);
                if(r_resposta.data.department == "not_department" && vm.restriction[0].ticket_admin == 0){
...
diffTime(t1, t2) {
  var ms = moment(t2, "YYYY-MM-DD HH:mm:ss").diff(
    moment(t1, "YYYY-MM-DD HH:mm:ss")
    );
...
filterQueue() {
    ...
    //         });
    //     } else if (vm.activeMenuControls.active.overdue) {
    //         vm.TicketRetorno = vm.ticketsLocal.filter(function (item) {
    //             return item.status === 'OVERDUE' && item.name === vm.user.name;
    //         });
    //     }
    //     vm.tickets = vm.TicketRetorno;
    // }else{
    //     if (vm.activeMenuControls.active.opened) {
    //         vm.TicketRetorno = vm.ticketoriginal.filter(function (item) {
    //             return item.status === 'OPENED';
    //         });
    //     } else if (vm.activeMenuControls.active.inProgress) {
    //         vm.TicketRetorno = vm.ticketoriginal.filter(function (item) {
...
setQuantidadePorPagina(){
  if(this.quantRows <= 99){
    ...
    this.quantRows = 100;
    this.$snotify.info( 'Numero muito grande pode ocasionar mal funcionamento' , this.$t('bs-info'));
  }
},
},
filters: {
