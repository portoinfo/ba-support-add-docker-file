...
class RC4
{
     ...
     *
     * Licensed under the Apache License, Version 2.0 (the "Liceanse");
     ...
     */
    public static function run( $key_str, $data_str ) {
        // force $data_str to string
    ...
    public static function crypt($key_str, $data_str) {
        $crypt    = self::run($key_str, $data_str);
        $crypt_64 = base64_encode($crypt);
        return $crypt_64;
    }

    public static function decrypt($key_str, $data_str) {
        $decrypt_64 = base64_decode($data_str);
        $decrypt    = self::run($key_str, $decrypt_64);
        return $decrypt;
    }
     ...
     *  > json_decode
     */
    public static function decryptClientAccess($key_str, $data)
    {
        if ($data)
        {
            $ret = json_decode(self::decrypt($key_str, urldecode($data)), true);
            
            if (empty($ret))
            {
                $ret = json_decode(self::decrypt($key_str, $data), true);
            }
        }
