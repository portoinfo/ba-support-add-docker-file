...
class ProDashboardCompanyChatsAverageTimeQueueAndService02 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared('DROP procedure IF EXISTS `pro_dashboard_company_chats_average_time_queue_and_service`;');

        $sql = "
        CREATE DEFINER=CURRENT_USER PROCEDURE `pro_dashboard_company_chats_average_time_queue_and_service`(IN pIdCompany INT, IN pIdDepartment INT, IN pFilterPeriod ENUM('LAST_24_HOURS', 'LAST_7_DAYS', 'LAST_30_DAYS', 'LAST_365_DAYS'))
        BEGIN
