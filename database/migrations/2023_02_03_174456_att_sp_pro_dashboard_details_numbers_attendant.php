...
class AttSpProDashboardDetailsNumbersAttendant extends Migration
{
    /**
     * Run the migrations.
     *
        ...
        DB::unprepared("DROP procedure IF EXISTS `pro_dashboard_details_numbers_attendant`;
            ...
            SET @SQL_Final = CONCAT('SELECT
                    ...
                    FROM (
                        ...
                        AND cu.user_auth_id = ', pIdAttendant,
                        IF(COALESCE(pInitialDate, 0) > 0, CONCAT(' AND t.created_at >= ''', pInitialDate, ''' '), ''),
                        IF(COALESCE(pFinalDate, 0) > 0, CONCAT(' AND t.created_at <= ''', pFinalDate, ''' '), ''),
