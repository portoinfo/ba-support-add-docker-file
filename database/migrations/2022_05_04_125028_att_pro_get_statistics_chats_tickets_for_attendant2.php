...
class AttProGetStatisticsChatsTicketsForAttendant2 extends Migration
{
    /**
     * Run the migrations.
     *
            ...
            SET @SQL_Final = 
                IF(pType = 'CHAT',
                    CONCAT('SELECT
                            ...
                            SUM(COALESCE(sub_work_times.opened, 0)) AS opened, SUM(COALESCE(sub_chat.finished, 0)) AS finished,
                        ...
                        ' WHERE company_id = ', pIdCompany, ' AND company_user.deleted_at IS NULL ',
                        IF(v_id_department > 0, CONCAT(' AND cucd.company_department_id = ', v_id_department), ''),
                        ' GROUP BY user_auth.id ORDER BY user_auth.name;'),
                        CONCAT('SELECT
                            user_auth.id, user_auth.name as attendant, company_user.last_login, company_user.id as company_user_id, 
                            SUM(COALESCE(sub_work_times.opened, 0)) AS opened, SUM(COALESCE(sub_ticket.finished, 0)) AS finished,
